...
function loadBanner(){
  ...
  if(id === 2401){
    ...
    "World Web Windows are half the story.\nNext part coming for holders soon.";
...
function updateCanvasView(){
  ...
  const scale =
  Math.min(canvasView.parentElement.clientWidth / canvasReal.width, 1);
...
function detectGreenSquares(){
  ...
  const data =
  ctxReal.getImageData(0,0,canvasReal.width,canvasReal.height).data;

  let y = 0;

  while(y < canvasReal.height){
